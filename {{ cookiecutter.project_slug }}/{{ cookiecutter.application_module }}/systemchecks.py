"""
The :py:mod:`{{ cookiecutter.application_module }}` application ships with some custom system
checks which ensure that required settings are present. These system checks are registered by
the :py:class:`~{{ cookiecutter.application_module }}.apps.Config` class's
:py:meth:`~{{ cookiecutter.application_module }}.apps.AssetsConfig.ready` method.
    ...
    for idx, name in enumerate(REQUIRED_SETTINGS):
        ...
        if value is None or value == '':
            errors.append(Error(
                ...
                id='{{ cookiecutter.application_module }}.E{:03d}'.format(idx + 1),
